...
app.get("/api/offers", async (req, res) => {
  ...
  try {
    ...
    const mappedOffers = offers.map(offer => ({
      ...offer,
      id: offer._id,
      company_name: offer.company_id ? (offer.company_id as any).name : 'Unknown'
    }));
  ...
  try {
    ...
    const mappedApps = apps.map(app => {
      const mapped: any = {
        ...app,
        id: app._id,
        student_name: app.student_id ? (app.student_id as any).name : 'Unknown',
        student_email: app.student_id ? (app.student_id as any).email : 'Unknown',
        offer_title: app.offer_id ? (app.offer_id as any).title : 'Unknown',
      };

      if (role === 'student' || role === 'admin') {
        mapped.company_name = app.offer_id && (app.offer_id as any).company_id
          ? (app.offer_id as any).company_id.name
          : 'Unknown';
      }
      return mapped;
    });
